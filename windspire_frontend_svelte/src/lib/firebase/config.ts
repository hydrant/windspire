...
function isFirebaseConfigValid(config: any): boolean {
    const hasAllFields = config.apiKey &&
        config.authDomain &&
        config.projectId &&
    ...
    const hasValidValues = !config.apiKey.includes('your-firebase') &&
...
if (browser && isFirebaseConfigValid(firebaseConfig)) {
    ...
    console.warn('Firebase configuration is invalid or contains placeholder values. Please set proper environment variables.');
    console.warn('Current config validation status:', {
        hasApiKey: !!firebaseConfig.apiKey,
        hasAuthDomain: !!firebaseConfig.authDomain,
        hasProjectId: !!firebaseConfig.projectId,
        apiKeyValid: !firebaseConfig.apiKey?.includes('your-firebase') && !firebaseConfig.apiKey?.includes('placeholder'),
